...
function bulma_pagination() {
	...
	if ( get_previous_posts_link() ) {
		printf( '%s', get_previous_posts_link( esc_html__( 'Previous', 'bulma' ) ) );
	}

	if ( get_next_posts_link() ) {
		printf( '%s', get_next_posts_link( esc_html__( 'Next', 'bulma' ) ) );
	}
 ...
 * @since 1.0.0
 */
function bulma_primary_navigation_fallback() {
	echo '<div class="navbar-menu">';
	echo '<a class="navbar-item" href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'bulma' ) . '</a>';
